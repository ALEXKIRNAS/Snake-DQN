...
from utils.experiance_replay import ExperienceReplay


class DeepQNetworkAgent(BaseAgent):
    ...
    def __init__(self, model, num_last_frames=4, memory_size=1000):
        ...
        Args:
            model: compiled DQN model.
            num_last_frames: the number of last frames the agent will consider.
            memory_size: memory size limit for experience replay
                        (-1 for unlimited).
        """

        input_shape_error = ('Model input shape should be (num_frames, ' 
                             'grid_size, grid_size)')
        output_shape_error = ('Model output shape should be (num_samples, '
                              'num_actions)')

        assert model.input_shape[1] == num_last_frames, input_shape_error
        assert len(model.output_shape) == 2, output_shape_error

        self.model = model
        self.num_last_frames = num_last_frames
        self.memory = ExperienceReplay(
            input_shape=(num_last_frames,) + model.input_shape[-2:],
    ...
    def get_last_frames(self, observation):
        """
        Get the pixels of the last `num_last_frames` observations,
        the current frame being the last.
    ...
    def train(self, env,
        ...
        Args:
            ...
            checkpoint_freq (int):
                the number of episodes after which a new model checkpoint
                will be created.
            exploration_range (tuple):
                a (max, min) range specifying how the exploration rate
        ...
        min_exploration_rate, max_exploration_rate = exploration_range
        ...
        for episode in range(num_episodes):
            ...
            summary = ('Episode {:5d}/{:5d} | Loss {:8.4f} | Exploration {'
                       ':.2f} | Fruits {:2d} | Timesteps {:4d} | '
                       'Total Reward {:4d}')
            
            print(summary.format(
                episode + 1, num_episodes, loss, exploration_rate,
                env.stats.fruits_eaten, env.stats.timesteps_survived,
