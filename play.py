...
from utils.argparser import HelpOnFailArgumentParser


def parse_command_line_args(args):
    """
    Parse command-line arguments and organize
    them into a single structured object.
    """

    parser = HelpOnFailArgumentParser(
        description='Snake AI replay client.',
        epilog='Example: play.py --agent dqn --model '
               'dqn-final.model --level 10x10.json'
    )
...
def play_cli(env, agent, num_episodes=10):
    ...
    Use the non-interactive command-line interface and print the
    summary statistics afterwards.
    ...
    print()
    print('Fruits eaten {:.1f} +/- stddev {:.1f}'.format(np.mean(fruit_stats),
                                                         np.std(fruit_stats)))
...
def main():
    ...
    model = load_model(parsed_args.model) if parsed_args.model else None
