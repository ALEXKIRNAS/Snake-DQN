...
from gameplay.field import Field, CellType
from gameplay.snake import Snake, SnakeActions, ALL_SNAKE_ACTIONS
from utils.timestep_result import TimestepResult
from utils.episode_statistics import EpisodeStatistics


class Environment(object):
    """
    Represents the RL environment for the Snake game that implements
    the game logic, provides rewards for the agent and keeps track of game
    statistics.
    """
    ...
    def __init__(self, config, verbose=1):
        ...
        Args:
            ...
            verbose (int): verbosity level:
                ...
                2 = same as 1, but also write a full log file containing the
    ...
    def seed(self, value):
        """
        Initialize the random state of the environment
    ...
    def record_timestep_stats(self, result):
        ...
        if self.verbose >= 1 and self.stats_file is None:
            self.stats_file = open('snake-env-{0}.csv'.format(timestamp), 'w')
            stats_csv_header_line = self.stats.to_dataframe()[:0].to_csv(
                index=None)
            print(stats_csv_header_line, file=self.stats_file, end='',
                  flush=True)

        # Create a blank debug log file.
        if self.verbose >= 2 and self.debug_file is None:
            self.debug_file = open('snake-env-{0}.log'.format(timestamp), 'w')
        ...
        if result.is_episode_end:
            if self.verbose >= 1:
                stats_csv_line = self.stats.to_dataframe().to_csv(header=False,
                                                                  index=None)
    ...
    def choose_action(self, action):
        ...
        elif action == SnakeActions.TURN_RIGHT:
    ...
    def timestep(self):
        ...
        if self.snake.get_next_point() == self.fruit:
            self.snake.grow()
            self.generate_fruit()
            old_tail = None
        ...
        self.field.update_field_repr(old_head, old_tail, self.snake.head)
